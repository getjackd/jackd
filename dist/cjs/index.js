...
var preserveCamelCase = (str, toLowerCase, toUpperCase, preserveConsecutiveUppercase2) => {
...
var postProcess = (input, toUpperCase) => {
  ...
  return input.replaceAll(
    ...
    (match, _pattern, offset) => ["_", "-"].includes(input.charAt(offset + match.length)) ? match : toUpperCase(match)
  ).replaceAll(
...
function camelCase(input, options) {
  ...
  const toLowerCase = options.locale === false ? (string) => string.toLowerCase() : (string) => string.toLocaleLowerCase(
    options.locale
  );
  const toUpperCase = options.locale === false ? (string) => string.toUpperCase() : (string) => string.toLocaleUpperCase(
    options.locale
  );
  if (input.length === 1) {
...
`);
    ...
    [
      ...
      (payload) => {
        ...
        const rawStats = import_yaml.default.parse(decodedString);
...
`),
    [
      ...
      (payload) => {
        const decodedString = new TextDecoder().decode(payload);
        return import_yaml.default.parse(decodedString);
      }
    ]
  );
